...
A primeira QUERY pega os dados DNO e SALARY da tabela EMPLOYEE enquanto que na segunda QUERY se houvesse dados idênticos de DNO e SALARY ele excluiria. Já a terceira QUERY nada mais é do que a segunda posta em ordem crescente para melhor visualização dos dados
...
Sim, elas são logicamente equivalentes porque seguem a regra da distributividade.
...
Sim, elas também têm o mesmo sentido lógico.

17. As seguintes cláusulas WHERE são logicamente equivalentes?

```         
1 - WHERE NOT (ACRES < 50 AND STATE = 'MA')
2 - WHERE NOT ACRES < 50 AND NOT STATE = 'MA'
3 - WHERE ACRES >= 50 OR STATE <> 'MA'
```

As cláusulas 1 e 3 são logicamente equivalentes entre si; a cláusula 2 não é equivalente a nenhuma das outras, pois trocou incorretamente o OR por AND ao aplicar a negação

18. As seguintes cláusulas WHERE são logicamente equivalentes?

```         
1 - WHERE NOT (ACRES < 50 OR STATE = 'MA')
2 - WHERE NOT ACRES < 50 OR NOT STATE = 'MA'
3 - WHERE NOT ACRES < 50 AND NOT STATE = 'MA'
```

As cláusulas 1 e 3 são equivalentes entre si. Já a cláusula 2 não é equivalente às outras, porque na negação o AND foi trocado por OR de forma errada.
